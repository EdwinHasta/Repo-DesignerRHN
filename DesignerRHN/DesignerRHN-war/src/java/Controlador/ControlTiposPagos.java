...
public class ControlTiposPagos implements Serializable {
    ...
    public void cancelarModificacion() {
        if (bandera == 1) {
            //CERRAR FILTRADO
            codigo = (Column) FacesContext.getCurrentInstance().getViewRoot().findComponent("form:datosTiposPagos:codigo");
            codigo.setFilterStyle("display: none; visibility: hidden;");
            descripcion = (Column) FacesContext.getCurrentInstance().getViewRoot().findComponent("form:datosTiposPagos:descripcion");
    ...
    public void verificarBorrado() {
        ...
        BigInteger contarProcesosTipoPago;

        try {
            System.err.println("Control Secuencia de ControlTiposPagos ");
            if (tipoLista == 0) {
                contarProcesosTipoPago = administrarTiposPagos.contarProcesosTipoPago(listTiposPagos.get(index).getSecuencia());
            } else {
                contarProcesosTipoPago = administrarTiposPagos.contarProcesosTipoPago(filtrarTiposPagos.get(index).getSecuencia());
            }
            if (contarProcesosTipoPago.equals(new BigInteger("0"))) {
                ...
                index = -1;
                contarProcesosTipoPago = new BigInteger("-1");
